...
class RaionController extends AbstractController
{
     ...
     * @return Response
     */
    public function edit(Oblast $oblast, string $id, Request $request, Edit\Handler $handler): Response
    {
        // $this->denyAccessUnlessGranted(OkrugAccess::MANAGE_MEMBERS, $okrug);


        $raion = $oblast->getRaion(new Id($id));
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $handler->handle($command);
                return $this->redirectToRoute('mesto.okrug.oblast.raion.show', ['id' => $oblast->getId(), 'id_s' => $id]);
            } catch (\DomainException $e) {
                $this->errors->handle($e);
                $this->addFlash('error', $e->getMessage());
            }
     ...
     * @return Response
     */
    public function delete(Oblast $oblast, string $id, Request $request, Remove\Handler $handler): Response
    {
    ...
    /**
     * @Route("/{id_s}", name=".show", requirements={"id_s"=Guid::PATTERN}))
     * @param Oblast $oblast
     * @return Response
     */
