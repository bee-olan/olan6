...
// не сработало all, попробую alll
    /**
     * @param Filter $filter
     * @param int $page
     * @param int $size
     ...
     * @return PaginationInterface
     */
    public function alll(Filter $filter, int $page, int $size, ?string $sort, ?string $direction): PaginationInterface
       {
        if (!\in_array($sort, [null, 't.id', 't.date', 'author_name', 'plemmatka_name', 't.name', 't.type', 't.plan_date', 't.progress', 't.priority', 't.status'], true)) {
            throw new \UnexpectedValueException('Cannot sort by ' . $sort);
        }
